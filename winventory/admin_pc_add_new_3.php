...
if ($software_audit == "n") {
  ...
  $result = mysql_query($sql);
  $sql = "UPDATE hotfix SET hotfix_timestamp = '$timestamp' WHERE hotfix_uuid = '$uuid' AND hotfix_timestamp = '$old_timestamp'";
  $result = mysql_query($sql);
  $sql = "UPDATE bho SET bho_timestamp = '$timestamp' WHERE bho_uuid = '$uuid' AND bho_timestamp = '$old_timestamp'";
  $result = mysql_query($sql);
} else {}
...
foreach ($input as $split) {
  ...
  if (substr($split, 0, 8) == "system03"){ 
	...
	$sql .= "system_country_code = '" . return_country($system_country_code) . "', system_description = '$system_description', ";
	$sql .= "date_system_install = '$date_system_install', system_organisation = '$system_organisation', ";
	$sql .= "system_language = '" . return_language($system_language) . "', system_registered_user = '$system_registered_user', ";
  ...
  if (substr($split, 0, 9) == "processor"){ 
    ...
    $processor_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 4) == "bios"){ 
    ...
    $bios_asset_tag = trim($extended[6]);
    $bios_timestamp = $old_timestamp;
	...
	if ($myrow['count'] == "0"){
      ...
      $sql .= "'$bios_sm_bios_version', '$bios_version', '$bios_asset_tag', '$timestamp', '$timestamp')";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
      $result = mysql_query($sql);
	} else {
	  // Already present in database - update timestamp
  ...
  if (substr($split, 0, 6) == "memory"){ 
    ...
    $memory_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 11) == "monitor_sys"){ 
    ...
    $monitor_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 9) == "harddrive"){ 
    ...
    $hard_drive_timestamp = $old_timestamp;
	...
	if ($myrow['count'] == "0"){
      ...
      $sql .= "hard_drive_scsi_port, hard_drive_size, hard_drive_timestamp, hard_drive_first_timestamp) VALUES (";
      $sql .= "'$uuid', '$hard_drive_caption', '$hard_drive_index', ";
      $sql .= "'$hard_drive_interface_type', '$hard_drive_manufacturer', '$hard_drive_model', ";
      $sql .= "'$hard_drive_partitions', '$hard_drive_scsi_bus', '$hard_drive_scsi_logical_unit', ";
      $sql .= "'$hard_drive_scsi_port', '$hard_drive_size', '$timestamp', '$timestamp')";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
      $result = mysql_query($sql);
	} else {
	  // Already present in database - update timestamp
	  $sql  = "UPDATE hard_drive SET hard_drive_timestamp = '$timestamp' WHERE hard_drive_index = '$hard_drive_index' AND ";
  ...
  if (substr($split, 0, 9) == "partition"){ 
    ...
    $partition_timestamp = $old_timestamp;
    ...
    $sql .= "partition_disk_index = '$partition_disk_index' AND partition_index = '$partition_index' AND ";
  ...
  // Disk Graph
  if (substr($split, 0, 10) == "graph_disk"){ 
    if ($verbose == "y"){echo "<h2>Disk Graph</h2><br />";}
    $extended = explode('^^^',$split);
	$disk_letter = trim($extended[1]);
	$disk_percent = trim($extended[2]);
    $sql  = "INSERT INTO graphs_disk (disk_uuid, disk_timestamp, disk_letter, disk_percent) VALUES (";
    $sql .= "'$uuid', '$timestamp', '$disk_letter', '$disk_percent')";
    if ($verbose == "y"){echo $sql . "<br />\n\n";}
    $result = mysql_query($sql);
  }

  // Optical Drives
  if (substr($split, 0, 7) == "optical"){ 
    ...
    $optical_drive_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 6) == "floppy"){ 
    ...
    $floppy_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 4) == "tape"){ 
    ...
    $tape_drive_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 8) == "keyboard"){ 
    ...
    $keyboard_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 6) == "shares"){ 
    ...
    $shares_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 6) == "mapped"){ 
    ...
    $mapped_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 7) == "l_group"){ 
    ...
    $groups_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 7) == "startup"){ 
    ...
    $startup_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 7) == "service"){ 
    ...
    $sql  = "SELECT count(service_uuid) AS count FROM service WHERE service_uuid = '$uuid' AND ";
    $sql .= "service_display_name = '$service_display_name' AND ";
    $sql .= "service_name = '$service_name' AND service_path_name = '$service_path_name' AND ";
    $sql .= "service_started = '$service_started' AND service_start_mode = '$service_start_mode' AND ";
    $sql .= "service_state = '$service_state' AND (service_timestamp = '$service_timestamp' OR service_timestamp = '$timestamp')";
    if ($verbose == "y"){echo $sql . "<br />\n\n";}
	$result = mysql_query($sql);
	$myrow = mysql_fetch_array($result);
	if ($verbose == "y"){echo "Count: " . $myrow['count'] . "<br />\n\n";}
	if ($myrow['count'] == "0"){
      ...
      $sql .= "service_timestamp, service_first_timestamp) VALUES (";
	  ...
	  // Already present in database - update timestamp
	  $sql  = "UPDATE service SET service_timestamp = '$timestamp' WHERE ";
      $sql .= "service_name = '$service_name' AND service_uuid = '$uuid' AND service_timestamp = '$service_timestamp' AND ";
      $sql .= "service_start_mode = '$service_start_mode' AND service_state = '$service_state' AND service_started = '$service_started'";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
      $result = mysql_query($sql);
	}
  ...
  if (substr($split, 0, 6) == "hotfix"){ 
    if ($verbose == "y"){echo "<h2>Hotfix</h2><br />";}
    $extended = explode('^^^',$split);
	$hotfix_description = trim($extended[1]);
	$hotfix_hot_fix_id = trim($extended[2]);
	$hotfix_installed_by = trim($extended[3]);
	$hotfix_service_pack_in_effect = trim($extended[4]);
    $hotfix_timestamp = $old_timestamp;
    $sql  = "SELECT count(hotfix_uuid) AS count FROM hotfix WHERE hotfix_uuid = '$uuid' AND ";
    $sql .= "hotfix_description = '$hotfix_description' AND hotfix_hot_fix_id = '$hotfix_hot_fix_id' AND ";
    $sql .= "hotfix_installed_by = '$hotfix_installed_by' AND hotfix_service_pack_in_effect = '$hotfix_service_pack_in_effect' AND ";
    $sql .= "(hotfix_timestamp = '$hotfix_timestamp' OR hotfix_timestamp = '$timestamp')";
    if ($verbose == "y"){echo $sql . "<br />\n\n";}
	$result = mysql_query($sql);
	$myrow = mysql_fetch_array($result);
	if ($verbose == "y"){echo "Count: " . $myrow['count'] . "<br />\n\n";}
	if ($myrow['count'] == "0"){
	  // Insert into database
      $sql  = "INSERT INTO hotfix (hotfix_uuid, hotfix_description, hotfix_hot_fix_id, hotfix_installed_by, ";
      $sql .= "hotfix_service_pack_in_effect, ";
      $sql .= "hotfix_timestamp, hotfix_first_timestamp) VALUES (";
      $sql .= "'$uuid', '$hotfix_description', '$hotfix_hot_fix_id', '$hotfix_installed_by', ";
      $sql .= "'$hotfix_service_pack_in_effect', ";
      $sql .= "'$timestamp', '$timestamp')";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
      $result = mysql_query($sql);
	  ...
	  // Already present in database - update timestamp
	  $sql = "UPDATE hotfix SET hotfix_timestamp = '$timestamp' WHERE hotfix_hot_fix_id = '$hotfix_hot_fix_id' AND hotfix_uuid = '$uuid' AND hotfix_timestamp = '$hotfix_timestamp'";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
      $result = mysql_query($sql);
	}
  }
  ...
  if (substr($split, 0, 8) == "software"){ 
	...
	if ($extended[10]) {$software_comments = trim($extended[10]);} else {$software_comments = "";}
    $software_timestamp = $old_timestamp;
    $sql  = "SELECT count(software_uuid) AS count FROM software WHERE software_uuid = '$uuid' AND ";
    $sql .= "software_name = '$software_name' AND (software_timestamp = '$software_timestamp' OR software_timestamp = '$timestamp')";
    if ($verbose == "y"){echo $sql . "<br />\n\n";}
    $result = mysql_query($sql);
	$myrow = mysql_fetch_array($result);
	if ($verbose == "y"){echo "Count: " . $myrow['count'] . "<br />\n\n";}
	if ($myrow['count'] == "0"){
	  ...
	  $sql .= "software_install_source, software_system_component, software_url, software_comment, software_timestamp, software_first_timestamp ) VALUES (";
      $sql .= "'$uuid','$software_name','$software_version', ";
	  $sql .= "'$software_location','$software_uninstall','$software_install_date','$software_publisher',";
	  $sql .= "'$software_install_source','$software_system_component','$software_url','$software_comments','$timestamp','$timestamp')";
	  if ($verbose == "y"){echo $sql . "<br />\n\n";}
      $result = mysql_query($sql);
	} else {
	  // Already in DB - flag as detected
	  $sql  = "UPDATE software SET software_timestamp = '$timestamp' WHERE software_uuid = '$uuid' AND software_name = '$software_name' AND software_timestamp = '$software_timestamp'";
	  if ($verbose == "y"){echo $sql . "<br />\n\n";}
      $result = mysql_query($sql);
	}
  }
  ...
  if (substr($split, 0, 8) == "fire_app"){ 
    ...
    $firewall_app_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 7) == "ms_keys"){ 
    ...
    $ms_keys_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 5) == "iis_2"){ 
    ...
    $iis_vd_timestamp = $old_timestamp;
  ...
  if (substr($split, 0, 5) == "iis_3"){ 
    ...
    $iis_ip_timestamp = $old_timestamp;
...
if ($country == "262"){$country_formatted="Réunion Island";}
