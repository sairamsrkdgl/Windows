...
foreach ($input as $split) {
  ...
  // Disk Graph
  if (substr($split, 0, 10) == "graph_disk"){ insert_graph_disk($split); }
  // Optical Drives
  ...
  // Hotfixes
  if (substr($split, 0, 6) == "hotfix"){ insert_hotfix($split); }
...
function insert_graph_disk ($split) {
...
function insert_printer ($split){
    ...
    if (strpos($printer_port_name,'IP_') !== false ) {
      ...
      print_r ($printer_name) . "<br />\n\n";
      if ($printer_name[6]){ $printer_caption = $printer_name[6]; }
    ...
    if (is_null($printer_timestamp)){
      $sql = "SELECT MAX(printer_timestamp) FROM printer WHERE printer_uuid = '$uuid'";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
	  $result = mysql_query($sql);
	  $myrow = mysql_fetch_array($result);
      if ($myrow["MAX(printer_timestamp)"]) {$printer_timestamp = $myrow["MAX(printer_timestamp)"];} else {$printer_timestamp = "";}
    } else {}
...
function insert_service ($split) {
    ...
    if (is_null($service_timestamp)) {
      $sql = "SELECT MAX(service_timestamp) FROM service WHERE service_uuid = '$uuid'";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
	  $result = mysql_query($sql);
	  $myrow = mysql_fetch_array($result);
      if ($myrow["MAX(service_timestamp)"]) {$service_timestamp = $myrow["MAX(service_timestamp)"];} else {$service_timestamp = "";}
      } else {}
...
function insert_hotfix ($split) {
    ...
    if (is_null($hotfix_timestamp)) {
      $sql = "SELECT MAX(hotfix_timestamp) FROM hotfix WHERE hotfix_uuid = '$uuid'";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
	  $result = mysql_query($sql);
	  $myrow = mysql_fetch_array($result);
      if ($myrow["MAX(hotfix_timestamp)"]) {$hotfix_timestamp = $myrow["MAX(hotfix_timestamp)"];} else {$hotfix_timestamp = "";}
      } else {}
...
function insert_software ($split) {
    ...
    if (is_null($software_timestamp)) {
      $sql = "SELECT MAX(software_timestamp) FROM software WHERE software_uuid = '$uuid'";
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
	  $result = mysql_query($sql);
	  $myrow = mysql_fetch_array($result);
      if ($myrow["MAX(software_timestamp)"]) {$software_timestamp = $myrow["MAX(software_timestamp)"];} else {$software_timestamp = "";}
      } else {}
...
//Output result
    echo "<br />&nbsp;<br />Page was generated in " . $totaltime . " seconds !";
